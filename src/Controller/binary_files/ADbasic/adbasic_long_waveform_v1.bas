...
' external event means rising edge goto event
' When CLK rising edge: inc count, else: IDLE
' This file follows the old approach (awg triggers adwin to count)
#Include ADwinGoldII.inc
DIM signal_count, ref_count, number_of_events AS LONG
DIM count_time, reset_time, iteration_number, i as LONG
...
init:
  ...
  UNTIL (i = 21)
event:
  Par_8 = Par_8 + 1                         ' current event number
  ...
  IF (Par_8 >= number_of_events) THEN
    Par_7=1
    Cnt_Enable(0)
    END
  ENDIF
